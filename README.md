...
* **`parquet_to_csv.py`:** A directory traversal script that locates hierarchical `.parquet` sensor files (e.g., Vivent biosignals). It extracts timestamp and amplitude data, converts Unix epoch timestamps into `dd/mm/yy hh:mm` format, and partitions the output into four equal CSV segments per original file.
* **`Preprocessing_and_Comparison.py`:** Ingests the continuous, segmented CSV files generated by the parquet converter. It applies sliding window segmentation to divide the continuous stream into uniform temporal instances, applies classification labels, and formats the data into a 3D NumPy array `(n_instances, n_channels, n_timepoints)` required by the `aeon` API.
